...
csps2020_org_out <- csps2020_org_scores %>%
  ...
  mutate(label = as_factor(label),
         value_label = scales::percent(value, accuracy = 1)) %>%
  select(measure, section, organisation, label, value, value_label)
...
Use this page to view the results for an individual organisation, use the filter controls to select an organisation and survey section.
...
govukhugo::govuk_datatable(
  csps2020_org_sd,
  col_names = c("Measure code", "Section", "Organisation", "Measure", "Value", "Data label"),
  search = FALSE,
  buttons = FALSE,
  col_defs = list(
    list(visible = FALSE, targets = list(0, 1, 2, 5))
  ),
